...
2. **有趣优先**：同等质量下，优先收录让人觉得「原来还能这么用」的项目——游戏与对战、模拟与具身实验、脑洞小工具、出人意料的真实落地（如 MedJev 的院内病历结构化）；纯目录、翻译、聚合类从严。
3. **可访问**：链接必须能打开。提交前请自己在浏览器里点一遍。
4. **优先级**：趣味与实验 > 可运行开源应用 > SDK / 集成 > 深度教程 > 社区讨论。
5. **不收录**：失效链接、纯营销落地页、搬运聚合号、无法核验的"据说很火"项目。
